    ...
    public void Configure(EntityTypeBuilder<Order> builder)
    {
        ...
        builder.OwnsOne(o => o.Pricing, pricing =>
        {
            pricing.OwnsOne(p => p.Subtotal, money =>
            {
                ...
                money.Property(m => m.Currency).HasColumnName("subtotal_currency").HasConversion(new CurrencyConverter());
            });

            pricing.OwnsOne(p => p.DeliveryFee, money =>
            {
                money.Property(m => m.Amount).HasColumnName("delivery_fee").HasColumnType("decimal(18,2)");
                money.Property(m => m.Currency).HasColumnName("delivery_fee_currency").HasConversion(new CurrencyConverter());
            });
            ...
            pricing.OwnsOne(p => p.DiscountAmount, money =>
            {
                ...
                money.Property(m => m.Currency).HasColumnName("discount_currency").HasConversion(new CurrencyConverter());
            });

            pricing.OwnsOne(p => p.TaxAmount, money =>
            {
                money.Property(m => m.Amount).HasColumnName("tax_amount").HasColumnType("decimal(18,2)");
                money.Property(m => m.Currency).HasColumnName("tax_currency").HasConversion(new CurrencyConverter());
            });

            pricing.OwnsOne(p => p.Total, money =>
            {
                money.Property(m => m.Amount).HasColumnName("total").HasColumnType("decimal(18,2)");
                money.Property(m => m.Currency).HasColumnName("total_currency").HasConversion(new CurrencyConverter());
            });
        });
