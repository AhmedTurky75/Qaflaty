...
public class GetCustomPageQueryHandler : IQueryHandler<GetCustomPageQuery, PageConfigurationDto>
{
    ...
    public GetCustomPageQueryHandler(IPageConfigurationRepository repository)
    {
        _repository = repository;
    }

    public async Task<Result<PageConfigurationDto>> Handle(
        GetCustomPageQuery query,
        CancellationToken cancellationToken)
    {
        var storeId = new StoreId(query.StoreId);
        var page = await _repository.GetByStoreIdAndSlugAsync(storeId, query.Slug, cancellationToken);
