    ...
    <section class="py-14 px-6 rounded-2xl bg-primary text-white text-center my-8">
      ...
      @if (externalLink()) {
        <a [href]="externalLink()" target="_blank" rel="noopener"
          class="inline-block px-8 py-3.5 bg-white text-primary rounded-lg font-semibold text-lg hover:bg-gray-100 transition-colors">
          {{ buttonText() }}
        </a>
      } @else if (internalLink()) {
        <a [routerLink]="internalLink()"
          class="inline-block px-8 py-3.5 bg-white text-primary rounded-lg font-semibold text-lg hover:bg-gray-100 transition-colors">
          {{ buttonText() }}
        </a>
      } @else {
        <button (click)="scrollToBuyBox()"
...
export class LandingCtaBandComponent {
  ...
  private buttonLink = computed(() => (this.content().buttonLink || '').trim());
  /** External URLs (http/https) open in a new tab. */
  externalLink = computed(() => /^https?:\/\//i.test(this.buttonLink()) ? this.buttonLink() : '');
  /** Internal app routes use the router. */
  internalLink = computed(() => {
    const link = this.buttonLink();
    return link && !this.externalLink() ? link : '';
  });
