...
/**
 * Movable inline "buy box" for product-landing pages. Content model:
 *   { heading:{en,ar}, buttonText:{en,ar}, showImage, showQuantity }
 * Collects variant + quantity and either adds to cart (opening the cart) or
 * "buys now" by adding and routing to the existing, proven checkout flow — it
 * deliberately reuses that flow rather than re-implementing order creation.
 * Renders only when a product is in context.
 */
@Component({
...
export class OrderFormComponent {
  ...
  needsVariant = computed(() => (this.product()?.hasVariants ?? false) && !this.selectedVariant());
  canBuy = computed(() => !!this.product() && !this.needsVariant());
