    ...
    <header class="bg-white shadow-sm sticky top-0 z-50">
      ...
      <div class="bg-gray-50 border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4 h-10 flex items-center justify-between text-sm">
          <div class="flex items-center gap-4 text-gray-600">
            <span class="text-sm">{{ store.currentStore()?.name }}</span>
          </div>
          <div class="flex items-center gap-3">
            @if (features.isSocialLinksEnabled() && features.socialLinks()) {
              <div class="flex items-center gap-2">
                @if (features.socialLinks()!.facebook) {
                  <a [href]="features.socialLinks()!.facebook!" target="_blank" rel="noopener" class="text-gray-600 hover:text-[var(--primary-color)] transition-colors">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                  </a>
                }
                @if (features.socialLinks()!.instagram) {
                  <a [href]="features.socialLinks()!.instagram!" target="_blank" rel="noopener" class="text-gray-600 hover:text-[var(--primary-color)] transition-colors">
                ...
                @if (features.socialLinks()!.twitter) {
                  <a [href]="features.socialLinks()!.twitter!" target="_blank" rel="noopener" class="text-gray-600 hover:text-[var(--primary-color)] transition-colors">
        ...
        <div class="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between gap-4">
          <a routerLink="/" class="flex items-center gap-2 flex-shrink-0">
            @if (store.currentStore()?.branding?.logoUrl) {
              <img [src]="store.currentStore()!.branding.logoUrl" alt="Logo" class="h-10 w-auto" />
            }
            <span class="font-bold text-xl text-gray-900">{{ store.currentStore()?.name }}</span>
          </a>

          @if (features.isProductSearchEnabled()) {
            <div class="hidden md:flex flex-1 max-w-2xl">
              <div class="relative w-full">
                <input type="search" [placeholder]="t('search')" class="w-full h-11 px-4 pr-12 rounded-lg border border-gray-300 focus:outline-none focus:border-[var(--primary-color)] focus:ring-1 focus:ring-[var(--primary-color)]" />
                <button class="absolute right-0 top-0 h-11 px-4 text-gray-400 hover:text-[var(--primary-color)]">
      ...
      @if (mobileMenuOpen()) {
        <nav class="md:hidden border-t border-gray-100 bg-white px-4 py-3 space-y-2">
          @if (features.isProductSearchEnabled()) {
            <div class="mb-4">
              <input type="search" [placeholder]="t('search')" class="w-full h-10 px-3 rounded-lg border border-gray-300 focus:outline-none focus:border-[var(--primary-color)] focus:ring-1 focus:ring-[var(--primary-color)]" />
            </div>
          }
