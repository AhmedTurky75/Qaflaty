...
@Component({
  ...
  template: `
    <div class="max-w-4xl mx-auto px-4 py-12">
      <h1 class="text-3xl font-bold text-gray-900 mb-8">{{ t('shipping') }}</h1>
      @if (page?.contentJson) {
        <div class="prose max-w-none" [innerHTML]="page.contentJson"></div>
      } @else {
        <p class="text-gray-600">Shipping and returns policy will be configured in the store builder.</p>
      }
    </div>
  `
})
